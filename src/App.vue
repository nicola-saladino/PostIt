...
// Colonne e post-it (ora vengono dal database)
const columns = ref([])
const postits = ref([])
...
const savePostit = async (postit) => {
  try {
    const postitData = {
      ...
      color: postit.color,
      column_id: postit.column,
      z_index: postit.zIndex
    }

    if (postit.id && postit.id > 1000000000000) { // Se è un ID temporaneo
      const { data, error: err } = await supabase
        .from('postits')
        .insert([postitData])
        .select()
...
const deletePostit = async (id) => {
  try {
    if (id > 1000000000000) { // Se è un ID temporaneo, rimuovi solo localmente
...
// Funzioni per gestire l'editing
...
<template>
  <div class="post-container">
    ...
    <div v-if="error" class="error">
      {{ error }}
      <button @click="() => { error = null; loadColumns(); loadPostits(); }">
        Riprova
      </button>
    </div>
...
/* Resto degli stili rimangono uguali... */
.post-container {
  position: absolute;
  top: 0;
  left: 0;
